...
class InstagramProvider:
    """Download de Reels, posts e IGTV do Instagram via yt-dlp."""
    ...
    def download(
        ...
        def _hook(d: dict[str, Any]) -> None:
            ...
            if status == "downloading" and progress_cb:
                ...
                msg = f"Baixando do Instagram… {pct:.1f}%"
                if speed:
                    msg += f" ({speed})"
                if eta:
                    msg += f" ETA {eta}"
                ...
                if progress_cb:
                    progress_cb(99.0, "Finalizando arquivo do Instagram…")
        ...
        if path is None or not path.exists():
            raise RuntimeError(
                "Download do Instagram concluído, mas o arquivo de vídeo não foi encontrado. "
                "Use um Reel, IGTV ou post com vídeo público (não carrossel só de fotos)."
            )

        title = None
        video_id = None
        if info:
            uploader = (info.get("uploader") or info.get("creator") or info.get("channel") or "").strip()
            ...
            if uploader and raw_title:
                ...
                title = raw_title or uploader or "instagram"
        ...
        if progress_cb:
            progress_cb(100.0, "Download do Instagram concluído")
        return path
