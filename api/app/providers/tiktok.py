...
class TikTokProvider:
    """Download de vídeos do TikTok via yt-dlp."""
    ...
    def download(
        ...
        def _hook(d: dict[str, Any]) -> None:
            ...
            if status == "downloading" and progress_cb:
                ...
                msg = f"Baixando do TikTok… {pct:.1f}%"
                if speed:
                    msg += f" ({speed})"
                if eta:
                    msg += f" ETA {eta}"
                ...
                if progress_cb:
                    progress_cb(99.0, "Finalizando arquivo do TikTok…")
        ...
        if path is None or not path.exists():
            raise RuntimeError(
                "Download do TikTok concluído, mas o arquivo de vídeo não foi encontrado. "
                "Posts só com fotos (slideshow) não geram MP4."
            )
        ...
        if info:
            ...
            if uploader and raw_title:
                ...
                title = raw_title or uploader or "tiktok"
        ...
        if progress_cb:
            progress_cb(100.0, "Download do TikTok concluído")
        return path
